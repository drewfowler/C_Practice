...
int  sum (int count,...)
{
  ...
  //Points to arguements
  va_list intArgumentPointer;

  //creates list of arguements from how many there
  va_start(intArgumentPointer, count);

  for (size_t i = 0; i < count; i++)
  {
    //Gets arguements and needs type of arguement
    nSum += va_arg(intArgumentPointer,int);
  }
  //Stops using arguement pointer
...
int min(int count,...)
{
  ...
  for (size_t i = 0; i < count; i++)
  {
      int test = va_arg(intArgumentPointer,int);
      if(i==0)
         nMin = test;
      else if(test<nMin)
        nMin = test;
